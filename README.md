...
> v0.4.0부터 **강화된 차단 우회 기능**이 포함되어 안정적인 사용이 가능합니다.
...
이 버전부터 동행복권 서버의 봇 탐지를 우회하기 위한 강력한 기능이 포함됩니다:
...
### 차단된 경우
- 이 버전(v0.4.0+)의 강화된 차단 우회 기능으로 대부분 해결됩니다
- 기존에 차단된 IP는 시간이 지나면 자동으로 해제됩니다 (보통 24시간~7일)
- VPN 사용 시 새 IP로 우회 가능
